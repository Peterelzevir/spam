...
PHONE_NUMBER = '+628xxx'  # Replace with your phone number for login
...
**OTP Feature:**
`.getotp` - Get latest OTP from Telegram official messages
...
async def get_otp(event):
    ...
    try:
        # Status update message
        status_msg = await event.respond("🔍 Searching for OTP messages from Telegram...")
        
        # Get the Telegram service notifications chat (777000)
        telegram_service = await client.get_entity(777000)
        
        # Get recent messages (increase limit to ensure we find OTP messages)
        messages = await client.get_messages(telegram_service, limit=20)
        
        # Filter messages likely containing OTP
        otp_messages = []
        otp_keywords = ["code", "código", "kod", "verification", "verificación", "password", "otp", "one-time"]
